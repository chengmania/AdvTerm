...
#[tauri::command]
fn pty_create(tab_id: String, app: AppHandle, state: State<Arc<PtyState>>) -> Result<(), String> {
...
fn pty_resize(tab_id: String, rows: u16, cols: u16, state: State<Arc<PtyState>>) -> Result<(), String> {
    ...
    if let Some(tab) = tabs.get(&tab_id) {
        tab.master.resize(PtySize { rows, cols, pixel_width: 0, pixel_height: 0 })
            .map_err(|e| e.to_string())?;
    }
    Ok(())
}
...
fn pty_close(tab_id: String, state: State<Arc<PtyState>>) -> Result<(), String> {
    // Dropping TabPty closes the master fd, sending SIGHUP to the shell
...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .manage(pty_state)
        .invoke_handler(tauri::generate_handler![pty_create, pty_write, pty_resize, pty_close])
